...
> **Status: In progress (2026-07-02).** C1–C6 are implemented, committed, and verified (`cargo test`, `cargo clippy -- -D warnings`, `cargo fmt --check`, plus a real `dotnet restore && dotnet build` and `dotnet format --verify-no-changes` against the full C2-C6 scaffold — all green). C4's OAuth1 RSA-SHA1 signing was additionally verified functionally (not just compiled): a throwaway RSA keypair signed and verified a base string with the exact `RSA.SignData`/`RSASignaturePadding.Pkcs1` primitives the generated `OAuth1Signer` uses. C5's `System.CommandLine` 2.0.9 usage and C6's `JsonSchema.Net`/`Microsoft.ML.OnnxRuntime`/`Microsoft.ML.Tokenizers`/MCP-SDK-tool-attribute usage were all written against current, live docs fetched during implementation (Microsoft Learn tutorials, the official MCP C# SDK devblog, json-everything.net) rather than assumed from training-data knowledge, and confirmed correct by the same real `dotnet build` — only one genuine compile error surfaced across the entire C6 surface (`JsonSchema.Evaluate` takes a `JsonElement`, not a `JsonNode`), fixed and reverified. C6's sqlite-vec native-extension loading (no NuGet wrapper exists) was verified with a real, separate `Microsoft.Data.Sqlite` k-NN query against the actual `vec0.dylib` release asset, producing correctly-ordered results — this is the "real gate" the plan's Verification section calls for, done early rather than deferred. `-l csharp` is **not yet** registered in `targets::build_registry()`; per this plan, that happens only once C8 is real and green. This plan covers adding the C#/.NET output target (`-l csharp`) to the mcpify generator, per `docs/architecture.md`'s "Target Language Roadmap" (§3). It assumes `docs/v1-implementation-plan.md` is complete — the shared pipeline, CLI, `GeneratorContext`, `McpServerTargetGenerator` trait, rollback, and `TargetRegistry` are reused as-is. This plan only covers the new, C#-specific per-target work.
