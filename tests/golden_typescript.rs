...
//! `INSTA_UPDATE=always cargo test --test golden_typescript`) after an
...
use mcpify::targets::typescript::steps::{
...
/// auth-scheme test file leaking in when its scheme wasn't discovered, etc).
...
async fn file_tree_single_basic_scheme() {
    ...
    insta::assert_debug_snapshot!("file_tree_single_basic_scheme", file_tree(&output_dir));
}
...
async fn file_tree_single_oauth2_scheme() {
    ...
    insta::assert_debug_snapshot!("file_tree_single_oauth2_scheme", file_tree(&output_dir));
}
...
async fn file_tree_all_four_schemes() {
    ...
    insta::assert_debug_snapshot!("file_tree_all_four_schemes", file_tree(&output_dir));
}
...
const CURATED_FILES: &[(&str, &str)] = &[
    ...
    ("register_tools_ts", "src/tools/register-tools.ts"),
];
...
async fn curated_file_contents_single_basic_scheme() {
    ...
    for (name, relative_path) in CURATED_FILES {
        ...
        insta::assert_snapshot!(format!("{name}_single_basic_scheme"), contents);
    }
}
