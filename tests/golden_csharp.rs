//! Golden/snapshot tests for the C# target's generation steps
...
//! (Story C8's real `dotnet restore`/`dotnet test` gate is exercised in
//! `e2e_generation.rs`, not here): these tests only assert that the
//! *content* mcpify writes hasn't changed, and stay fast/offline by
//! skipping that step entirely.
//!
//! Review changes with `cargo insta review` (or accept them directly with
//! `INSTA_UPDATE=always cargo test --test golden_csharp`) after an
...
use mcpify::targets::csharp::steps::{
...
/// auth-strategy file leaking in when its scheme wasn't discovered, etc).
...
async fn file_tree_single_basic_scheme() {
    ...
    insta::assert_debug_snapshot!(
        "csharp_file_tree_single_basic_scheme",
...
async fn file_tree_single_oauth2_scheme() {
    ...
    insta::assert_debug_snapshot!(
        "csharp_file_tree_single_oauth2_scheme",
        file_tree(&output_dir)
    );
}
...
async fn file_tree_all_four_schemes() {
    ...
    insta::assert_debug_snapshot!("csharp_file_tree_all_four_schemes", file_tree(&output_dir));
}

/// Full-content snapshots of a curated subset of "interesting" files,
/// rather than all emitted files, to keep review signal-to-noise
/// reasonable when a template changes. `Out.csproj`'s filename is
/// namespace-derived (PascalCase of the `out` output directory name).
const CURATED_FILES: &[(&str, &str)] = &[
    ("out_csproj", "Out.csproj"),
    ("config_cs", "Core/Config.cs"),
    ("auth_manager_cs", "Auth/AuthManager.cs"),
    ("mcp_server_cs", "Core/McpServer.cs"),
];
...
async fn curated_file_contents_single_basic_scheme() {
    ...
    for (name, relative_path) in CURATED_FILES {
        ...
        insta::assert_snapshot!(format!("csharp_{name}_single_basic_scheme"), contents);
    }
}
