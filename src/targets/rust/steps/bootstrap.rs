...
/// plain, non-executable files (matching `scripts/coverage.sh`, which sidesteps
/// this by always being invoked as `bash scripts/coverage.sh`), so the
/// pre-commit hook needs its own explicit chmod after being written.
#[cfg(unix)]
...
/// (Story R3). `mcp_store.db` is already written directly into
/// `ctx.output_dir` by the shared pipeline (Story 5/6), so this step only
/// lays out `src/` and the root-level project files around it. Mirrors
/// `targets::typescript::steps::bootstrap::bootstrap_project` exactly, one
/// concern-subdirectory per module family — the same folder-per-concern
/// shape as v1, since Rust's module system supports it just as well and
/// keeping the two targets' trees shaped alike is worth more than a
/// Rust-flatter alternative would save.
...
mod tests {
    ...
    #[tokio::test]
    async fn cargo_toml_references_the_binary_and_package_name() {
        let parent = tempfile::tempdir().unwrap();
        let dir = output_dir(&parent);
        let ctx = ctx_with_schemes(dir.clone(), Vec::new());
        ...
        let contents = tokio::fs::read_to_string(dir.join("Cargo.toml"))
            .await
            .unwrap();
