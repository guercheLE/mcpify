...
async fn run_command(cwd: &Path, program: &str, args: &[&str], label: &str) -> Result<()> {
    ...
    let (stdout, stderr, status) = timeout(GO_TIMEOUT, async {
        tokio::try_join!(
...
/// watching `mcpify` run sees the subprocess's own output live instead of
/// a multi-minute silence), while always accumulating the full bytes —
