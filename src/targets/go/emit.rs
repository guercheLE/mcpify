...
/// (Stories G2-G7) funnels through this one helper, so template/IO error
/// context is centralized in one place instead of repeated at each call
/// site. Identical in shape to `targets::csharp::emit::render_and_write` and
/// every other target's equivalent — deliberately duplicated rather than
/// shared, since each target's `Tera` instance holds a disjoint template set
/// and sharing this one generic function across them would be the only
/// coupling between otherwise fully independent target implementations.
